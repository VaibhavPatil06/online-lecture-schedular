...
// Add a new lecture
export const addLecture = createAsyncThunk('lectures/addLecture', async (lecture: Omit<Lecture, 'id'>) => {
  const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/lectures/add-lecture`, lecture);
  return response.data;
});

// Delete a lecture
export const removeLecture = createAsyncThunk('lectures/removeLecture', async (lectureId: string) => {
  await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/v1/lectures/delete-lecture?lectureId=${lectureId}`);
  return lectureId;
});
